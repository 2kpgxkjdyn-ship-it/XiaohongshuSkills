...
`semi_auto_comment.py` 用于“搜索候选笔记 -> 人工确认 -> 发布评论”的半自动流程。默认会读取 `comment_templates.txt`，一行一条评论模板；发布前仍需人工输入 `p` 确认，支持 `r` 换一句、`e` 编辑、`s` 跳过、`q` 退出。

```bash
# 按关键词搜索候选笔记，逐条人工确认评论
python semi_auto_comment.py --keyword "找品" --limit 10

# 不等待下一条（不建议高频使用）
python semi_auto_comment.py --keyword "找品" --limit 5 --no-wait

# 使用自定义配置
python semi_auto_comment.py --config config/semi_auto_comment.json --keyword "私域选品"

# 如需临时允许重复触达日志中已有笔记
python semi_auto_comment.py --keyword "找品" --allow-repeat
```

可复制 `config/semi_auto_comment.example.json` 为 `config/semi_auto_comment.json` 后调整筛选规则。支持配置项包括：标题加分词、标题扣分词、必须包含词、屏蔽词、最低分、每日成功上限、是否跳过历史触达、搜索筛选项。

日志会记录到 `comment_log.csv`，默认保留 `timestamp`、`keyword`、`note_url`、`comment`、`status`。如果检测到旧版日志表头，新记录会自动写入 `comment_log_v2.csv`，避免破坏旧日志。脚本会从日志链接中提取 `note_id` 去重，避免同一笔记因 `xsec_token` 变化而重复触达。
队列发布前还会重新读取日志做最终去重检查；如果队列中混入已经触达过的笔记，会自动标记为 `skipped / duplicate_logged_note`，不会继续发布。终端审查会区分 `待审` 和 `已批准待发布`；批准一条后待审数减少、已批准待发布数增加。审查界面不显示评分，完成记录会显示成功/失败状态和处理时间。

说明：半自动评论脚本不会在发布后再次回读详情检查评论是否出现；只要底层发布命令成功返回，就按成功记录。这样可以减少额外页面访问和二维码/风控触发概率。

脚本运行时会追加写入 `run_status.log`，记录当前关键词、搜索进度、队列生成、等待输入、发布结果等状态。若在另一个窗口运行脚本，可通过查看该文件了解当前进度。

#### 队列模式

队列模式会先生成待审 CSV，不会立即发布。随后用 `--review-queue` 在终端里批准、跳过或编辑评论，再执行发布命令。
队列表只保留 `note_id`、链接、token、评分、建议评论、状态等可稳定提取字段；标题和作者不写入队列。

```bash
# 1. 生成 10 条待审候选，不发布
python semi_auto_comment.py --make-queue --keyword "找货源" --limit 10

# 2. 在终端审查队列：a 批准当前，all 全部批准，s 跳过，e 改评论
python semi_auto_comment.py --review-queue

# 3. 只发布 approve=1 且 status 为空或 pending 的记录
python semi_auto_comment.py --publish-approved

# 可选：追加到现有队列，而不是覆盖
python semi_auto_comment.py --make-queue --append-queue --keyword "私域选品" --limit 10
```

底层命令支持结构化输出：

```bash
python scripts/cdp_publish.py --json-output --reuse-existing-tab search-feeds --keyword "找品"
python scripts/cdp_publish.py --json-output --reuse-existing-tab post-comment-to-feed \
  --feed-id FEED_ID \
  --xsec-token XSEC_TOKEN \
  --content "评论内容"
```

启用 `--json-output` 时，最终结果会以单行 JSON 输出到 stdout，普通运行日志会转到 stderr，便于上层脚本稳定解析。
...
如果不想每次在 PowerShell 里复制命令，可以启动本地 Web 工作台：
...
- 输入关键词和候选数量，生成 `review_queue.csv`
- 在网页里查看候选、修改建议评论、批准、跳过、批量批准
- 点击“发布已批准”，调用 `semi_auto_comment.py --publish-approved`
- 显示任务输出、`run_status.log` 运行状态和队列统计
- 队列统计区分 `待审` 和 `已批准待发布`；批准后待审数减少、已批准待发布数增加。候选卡片不显示评分，完成记录会显示处理时间和失败原因
- 发布控制会显示每日发布上限、今日成功数、剩余额度，以及“达到上限/无可发布任务”等原因提示
- 快捷启动、重启、关闭 Chrome，并检查登录状态

同一时间只允许一个后台任务运行，避免重复发布或覆盖队列。底层仍然复用现有 `semi_auto_comment.py`、`review_queue.csv` 和 `run_status.log`，所以也可以继续用原来的 PowerShell 命令混合操作。
