...
当用户需要“自动搜索、人工确认、半自动发表评论”时，优先使用 `semi_auto_comment.py`，不要直接批量调用 `post-comment-to-feed`。该脚本会搜索候选笔记、按配置评分和过滤、从日志链接中提取 `note_id` 跳过已触达笔记，并在用户输入 `p` 后才发布评论。

```bash
# 默认读取 comment_templates.txt，一行一条评论模板
...
交互键位：`p` 发布，`r` 换一句，`e` 编辑，`s` 跳过，`q` 退出。

配置模板位于 `config/semi_auto_comment.example.json`。可复制为 `config/semi_auto_comment.json` 后调整：标题加分词、标题扣分词、必须包含词、屏蔽词、最低分、每日成功上限、是否跳过历史触达、搜索筛选项。

说明：半自动评论脚本不会在发布后再次回读详情检查评论是否出现；只要底层发布命令成功返回，就按成功记录，以减少额外页面访问和二维码/风控触发概率。
发布队列前必须依赖 `comment_log.csv` 做最终去重；脚本会从日志链接提取 `note_id`，遇到已触达笔记时标记为 `skipped / duplicate_logged_note`，不要再次发布。

运行状态会追加写入 `run_status.log`，可用于在用户自己的 PowerShell 窗口运行时从文件侧查看当前进度。

队列模式适合批量预审：先生成 `review_queue.csv`，由用户在终端审查模式中批准记录，再只发布已批准记录。审查模式支持 `a` 批准当前、`all` 全部批准、`s` 跳过、`e` 改评论、`n` 下一条、`q` 退出。审查界面要区分 `待审` 和 `已批准待发布`，批准后待审数减少、已批准待发布数增加；不要显示评分，完成记录显示成功/失败状态和处理时间。
队列表只保留可稳定提取的字段，不写入标题和作者。
...
# 只发布 approve=1 且 status 为空或 pending 的记录
python semi_auto_comment.py --publish-approved
```

说明：`scripts/cdp_publish.py` 支持全局参数 `--json-output`。启用后，最终结果以单行 JSON 输出到 stdout，普通运行日志输出到 stderr，适合作为上层自动化脚本的稳定解析接口。
...
当用户想要“工作台”“控制台”“网页审批”“不用每次复制 PowerShell 命令”时，优先启动本地 Web 工作台：
...
工作台会复用 `semi_auto_comment.py`、`review_queue.csv`、`comment_log.csv` 和 `run_status.log`，支持关键词生成待审队列、网页批准/跳过/修改评论、发布已批准队列、查看任务输出和运行状态，并提供 Chrome 启停与登录检查快捷按钮。网页队列统计要区分 `待审` 和 `已批准待发布`；批准后待审数减少、已批准待发布数增加。候选卡片不要显示评分，完成记录显示处理时间和失败原因。发布控制要显示每日发布上限、今日成功数、剩余额度，以及达到上限或无可发布任务等原因提示。
